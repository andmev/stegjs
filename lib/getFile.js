...
exports.byURI = function (imgURI, callback) {
    ...
    request(imgURI)
        ...
        .on('response', res => res.statusCode >= 400 ? callback('Something wrong with URL or server', null) : fs.createWriteStream(filename))
        .on('close', () => callback(null, `${process.cwd()}/${filename}`))
};
