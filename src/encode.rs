...
fn encode_image(file_path: &str, msg: &str, step: &str, out: &str) -> Result<(), Box<dyn Error>> {
  ...
  if !encoding_complete {
    return Err(
      "A very long message or a wide step! This amount of text does not fit in this picture."
        .into(),
    );
  }
